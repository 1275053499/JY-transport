...
-(void)initPickerView{
    ...
    pushImageName = @"icon_tianjiatupian_xuxian";
    ...
    [self updatePhotos:_arrSelected];
...
//            [cell setBigImageViewWithImage:_imageArray[index]];
        }
...
- (void)deletePhoto:(UIButton *)sender{
    ...
    _imgArr = [self getBigImageArrayWithALAssetArray:_arrSelected];
...
-(void)getSelectImageWithALAssetArray:(NSArray *)ALAssetArray thumbnailImageArray:(NSArray *)thumbnailImgArray{
    ...
    [self updatePhotos:ALAssetArray];
}


- (void)updatePhotos:(NSArray *)ALAssetArray{
    
    //（ALAsset）类型 Array
    _arrSelected = [NSMutableArray arrayWithArray:ALAssetArray];
    //正方形缩略图 Array
    //    _imageArray = [NSMutableArray arrayWithArray:thumbnailImgArray];
    NSArray *arr = [self getBigImageArrayWithALAssetArray:_arrSelected];
    
    _imageArray = [NSMutableArray arrayWithArray:arr];
...
- (void)updatePickerViewFrameY:(CGFloat)Y{
    ...
    _collectionFrameY = Y;
    _pickerCollectionView.frame = CGRectMake(0, Y,ScreenWidth, (((float)ScreenWidth -64.0) /4.0 +20.0)* ((int)(_arrSelected.count)/4 +1)+20.0);
}
...
- (NSArray*)getBigImageArray:(NSArray*)arr{
    
    return [self getBigImageArrayWithALAssetArray:arr];
}
