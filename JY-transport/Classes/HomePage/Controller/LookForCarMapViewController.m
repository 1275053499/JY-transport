...
@interface LookForCarMapViewController ()<MAMapViewDelegate,BMKLocationServiceDelegate,BMKGeoCodeSearchDelegate,BMKMapViewDelegate,UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate,TLCityPickerDelegate,UINavigationControllerDelegate,BMKPoiSearchDelegate>
...
@implementation LookForCarMapViewController
...
//        _pointAnnotation.lockedScreenPoint= self.mapView.center;
//        
...
- (UIButton *)address
{
    if (!_address) {
        ...
        [_address setTitleColor:RGB(51, 51, 51) forState:(UIControlStateNormal)];
...
- (UITableView *)searchTableView
{
    if (!_searchTableView) {
        ...
        _searchTableView.frame = CGRectMake(0, 78, ScreenWidth , ScreenHeight * 0.45);
        ...
        _searchTableView.separatorInset = UIEdgeInsetsMake(0, 0, 0, 0);
...
-(void)confirmAddress
{
    ...
    if (self.delegate && self.resultData != nil) {
        ...
        if (_markClickCell == YES) {
            ...
            [self.delegate cityWideForDayAndHalfDay:_address.titleLabel.text];
            [self backToPage];
            _markClickCell = NO;
        }else{
            // 代理回调
            ...
            [self.delegate cityWideForDayAndHalfDay:amp.city];
            [self backToPage];
        }
